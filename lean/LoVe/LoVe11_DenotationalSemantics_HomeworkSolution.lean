...
| x :: xs, ys =>
  choice (fun b ↦
    if b then
      nondetPortmanteau xs ys >>= (fun zs ↦ pure (List.cons x zs))
    else
      if startsWith (x :: xs) ys then pure ys else fail)
...
def listSem {α : Type} : Nondet α → List α
  | just x   => [x]
  | fail     => []
  | choice k => listSem (k false) ++ listSem (k true)
...
noncomputable def optionSem {α : Type} : Nondet α → Option α
  | just a   => Option.some a
  | fail     => Option.none
  | choice k =>
    match optionSem (k false) with
    | Option.none   => optionSem (k true)
...
theorem List_Option_compat {α : Type} :
  ...
  | choice k =>
    by
      simp [listSem, optionSem]
      simp [List_Option_compat (k false), List_Option_compat (k true)]
      cases listSem (k false) <;>
        simp
...
def denote : Stmt → Set (State × State)
  ...
  | Stmt.unlessDo S B => (Id ⇃ B) ∪ (denote S ⇃ (fun s ↦ ¬ B s))
  | Stmt.whileDo B S  =>
    lfp (fun X ↦ ((denote S ◯ X) ⇃ B) ∪ (Id ⇃ (fun s ↦ ¬ B s)))
  | Stmt.doWhile S B  =>
    denote S ◯ lfp (fun X ↦ ((denote S ◯ X) ⇃ B) ∪ (Id ⇃ (fun s ↦ ¬ B s)))
...
theorem Monotone_iterate {α : Type} (f : Set α → Set α) (hf : Monotone f) :
  ...
  by
    simp [Monotone]
    intro i j hle a ha
    induction i generalizing a j with
    | zero => simp [iter] at ha
    | succ i' ih =>
      cases j with
      | zero    => simp at hle
      | succ j' =>
  ...
  by
    ...
    · apply Union_le (lfp f)
      ...
      | succ i' ih =>
        ...
        apply Monotone_of_Continuous f hf
        apply ih
